...
class Challenge_model extends CI_Model{
	...
	function find($key=""){
		$this->db->select("*");
		if ( empty($key) == FALSE )
			$this->db->where("challenge_id", $key);
		$q = $this->db->get("vw_challenge");
		if ( $q->num_rows() == 0 )
			return FALSE;
		return $q->result();
	}
	...
	function _get_applicant_query(){
		$__order 			= array('challenge_id' => 'ASC');
		$__column_search 	= array('challenge_id', 'challenge_name');
		$__column_order     = array('challenge_id', 'challenge_name');

		$this->db->select('*');
		$this->db->from('vw_challenge');
