$( function () {
	...
	$('.btn-add-challenge').on('click', function() {
		$('#default-modal').modal('show');
	});

	var t = $('#challengeTbl').DataTable({
		"processing" : true,
		"language": {
		...
		"order": [], 
		"ajax": {
			"url": base_url + "challenge/view_challenge",
			"type": "POST"
		},
		"searchDelay" : 750,
		"columnDefs"	: [{
	...
	$("#challenge-form").ajaxForm({
		dataType: "json",
		url : base_url + 'challenge/save',
		...
		$.ajax({
			...
			success: function(data){
				if (data.type != 'done'){
					...
					$("#id").val ( row_id );
					$("#challenge_name").val ( data.msg[0].challenge_name );
					$("#challenge_desc").val ( data.msg[0].challenge_desc );
					$('#challenge_files').removeAttr('required');
					$("#type").val ( "update" );
